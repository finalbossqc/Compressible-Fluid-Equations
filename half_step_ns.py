...
def calculate_step(u, v, f1, f2, rho, mu, dx, dt):
    ...
    while (m1 < N):
        m2 = 0
        while (m2 < N):
            if (m1 == 0 or m2 == 0 or m1 == N//2 or m2 == N//2):
                q_hat[m1,m2] = 0
            else:
    ...
    while (m1 < N):
        ...
        while (m2 < N):
            ...
            u_hat[m1,m2] = (w_hat_1[m1,m2] - (dt/rho) * D_hat_1[m1,m2] * q_hat[m1,m2]) / (1 - (dt*mu)/(2*rho)*L_hat[m1,m2])
            v_hat[m1,m2] = (w_hat_2[m1,m2] - (dt/rho) * D_hat_2[m1,m2] * q_hat[m1,m2]) / (1 - (dt*mu)/(2*rho)*L_hat[m1,m2])
            m2 += 1
        m1+=1
...
def main():
    ...
    N = int(100/0.5)
    anim_u, anim_v = solve_ns_explosion(1, 1000, 100, 0.5, 0.01, 20, N//2, N//2, 100, 100, 6, 2, 3, 0)
